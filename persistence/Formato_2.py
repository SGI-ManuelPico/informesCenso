...
def determinar_mayoria_unidad(unidades):
    """
    Esta función determina la unidad mayoritaria en una lista de unidades.
    """
    return max(set(unidades), key=unidades.count)

df2 = xl.parse(sheet_name='FORMATO 2. AGROPECUARIO', header=None)
df2 = df2.drop(columns=[0, 1, 2, 3])
df2_T = df2.transpose()
df2_T.columns = df2_T.iloc[0]
df2_T = df2_T.drop(df2_T.index[0])
df2_T.columns = df2_T.columns.str.strip()     
df2_T.columns = pd.io.common.dedup_names(df2_T.columns, is_potential_multiindex=False)
df_enc2 = df2_T.reset_index(drop = True)

def llenar_form_2(ws, df_fila):
    ...
    unidad_area_cultivada_mayoria = determinar_mayoria_unidad(unidades_area_cultivada)
    unidad_establecimiento_mayoria = determinar_mayoria_unidad(unidades_establecimiento)
    unidad_mantenimiento_mayoria = determinar_mayoria_unidad(unidades_mantenimiento)
    unidad_cosecha_mayoria = determinar_mayoria_unidad(unidades_cosecha)
    unidad_precio_mayoria = determinar_mayoria_unidad(unidades_precio)
    unidad_autoconsumo_mayoria = determinar_mayoria_unidad(unidades_autoconsumo)
...
# Llenar raza y # de cabezas
    ...
    unidad_mayoritaria_precio_venta = determinar_mayoria_unidad(unidades_precio_venta)
    unidad_mayoritaria_costo_produccion = determinar_mayoria_unidad(unidades_costo_produccion)
...
if __name__ == "__main__":
    for index, row in df_enc2.iterrows():
        print(f"Processing row {index + 1}")
        wb = load_workbook(form)
        ws = wb.active
        
        llenar_form_2(ws, row)
